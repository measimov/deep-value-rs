...
class ApiInfraReadinessReportTests(unittest.TestCase):
    ...
    def test_readiness_report_is_stable_and_includes_inventory(self):
        ...
        self.assertIn("anns", payload["missing_infrastructure_by_category"]["object_store_index"])
        self.assertIn("anns", payload["missing_infrastructure_by_category"]["object_download_validation"])
        self.assertIn("news", payload["missing_infrastructure_by_category"]["text_dedup_policy"])
        self.assertIn("stk_mins", payload["missing_infrastructure_by_category"]["needs_intraday_bucket"])
        self.assertIn("stk_mins", payload["missing_infrastructure_by_category"]["intraday_bucket_partition"])
        self.assertIn("stk_mins", payload["missing_infrastructure_by_category"]["compaction_executor"])
        self.assertIn("stk_mins", payload["missing_infrastructure_by_category"]["query_benchmark"])
        self.assertIn("stk_mins", payload["missing_infrastructure_by_category"]["storage_capacity_plan"])
        self.assertIn("realtime_quote", payload["missing_infrastructure_by_category"]["needs_realtime_policy"])
        self.assertIn("realtime_quote", payload["missing_infrastructure_by_category"]["realtime_policy"])
        self.assertIn("remote_disaster_recovery", payload["missing_infrastructure_by_category"])
        self.assertIn("postgres_derived_layer", payload["missing_infrastructure_by_category"])
        ...
        self.assertEqual(payload["code_date_matrix_existing_status"], "implemented")
        self.assertEqual(payload["period_planner"], "plan_only")
        self.assertEqual(payload["code_period_matrix_planner"], "plan_only")
        self.assertEqual(payload["pit_safety_metadata"], "implemented")
        self.assertEqual(payload["pit_readiness_report"], "implemented")
        self.assertEqual(payload["object_text_planner"], "plan_only")
        self.assertFalse(payload["object_download_execution"])
        self.assertEqual(payload["intraday_bucket_planner"], "plan_only")
        self.assertFalse(payload["intraday_execution"])
        self.assertEqual(payload["compaction_planner"], "plan_only")
        self.assertFalse(payload["compaction_execution"])
        self.assertEqual(payload["storage_estimate"], "implemented")
        self.assertEqual(payload["rate_policy_report"], "implemented")
        self.assertEqual(payload["endpoint_enable_checklist"], "implemented")
        self.assertFalse(payload["executable_code_loop"])
        self.assertFalse(payload["executable_code_date_matrix"])
        self.assertFalse(payload["executable_period_loop"])
        self.assertFalse(payload["executable_code_period_loop"])
        self.assertFalse(payload["financial_execution"])
        self.assertEqual(payload["max_safe_code_plan_limit"], 20)
        self.assertEqual(payload["max_safe_code_limit"], 20)
        self.assertEqual(payload["max_safe_date_limit"], 20)
        self.assertEqual(payload["max_safe_period_limit"], 20)
        self.assertEqual(payload["max_safe_candidate_jobs"], 100)
        self.assertIn("PIT safe usable_after generation", payload["missing_for_execution"])
    ...
    def test_cli_table_output_is_read_only(self):
        ...
        self.assertIn("period_planner", result.stdout)
        self.assertIn("pit_safety_metadata", result.stdout)
        self.assertIn("object_text_planner", result.stdout)
        self.assertIn("intraday_bucket_planner", result.stdout)
        self.assertIn("compaction_planner", result.stdout)
        self.assertIn("endpoint_enable_checklist", result.stdout)
